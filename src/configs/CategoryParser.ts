...
const defaultCategory: CategoryData[] = CategoryData.default; // 访问实际的数组
let currentCategory: CategoryData[] = defaultCategory;
...
export async function updateCurrentCategory() {
    try {
        ...
        const response = await fetchCustomCategory();
        const data = response.data.custom as CategoryData[]
        console.log(response.data)
        if (data != null && data.length > 0) {
            currentCategory = response.data.custom as CategoryData[]
